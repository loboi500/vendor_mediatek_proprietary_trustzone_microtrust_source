...
void addClientAndAppData(const std::vector<uint8_t>& appId,
...
BeanpodKeyMintDevice::BeanpodKeyMintDevice(BeanpodKeymaster* impl)
	: impl_(impl) {
		securityLevel_ = SecurityLevel::TRUSTED_ENVIRONMENT;
}

BeanpodKeyMintDevice::~BeanpodKeyMintDevice() {}

ScopedAStatus BeanpodKeyMintDevice::getHardwareInfo(KeyMintHardwareInfo* info) {
	info->versionNumber = 1;
	info->securityLevel = securityLevel_;
	info->keyMintName = "KeyMint";
	info->keyMintAuthorName = "Beanpod";
	info->timestampTokenRequired = false;
	return ScopedAStatus::ok();
}

ScopedAStatus BeanpodKeyMintDevice::addRngEntropy(const vector<uint8_t>& data) {
	if (data.size() == 0) {
		return ScopedAStatus::ok();
	}
...
ScopedAStatus BeanpodKeyMintDevice::generateKey(const vector<KeyParameter>& keyParams,
	...
	bool include_keystore_enforced = true;
	if (response.unenforced.empty()) {
		include_keystore_enforced = false;
	}
	creationResult->keyCharacteristics = convertKeyCharacteristics(
		securityLevel_, request.key_description, response.unenforced, response.enforced, include_keystore_enforced);
	creationResult->certificateChain = convertCertificateChain(response.certificate_chain);
	return ScopedAStatus::ok();
}

ScopedAStatus BeanpodKeyMintDevice::importKey(const vector<KeyParameter>& keyParams,
...
ScopedAStatus
BeanpodKeyMintDevice::importWrappedKey(const vector<uint8_t>& wrappedKeyData,
	...
	creationResult->keyCharacteristics = convertKeyCharacteristics(
		securityLevel_, request.additional_params, response.unenforced, response.enforced, include_keystore_enforced);
	creationResult->certificateChain = convertCertificateChain(response.certificate_chain);

	return ScopedAStatus::ok();
}

ScopedAStatus BeanpodKeyMintDevice::upgradeKey(const vector<uint8_t>& keyBlobToUpgrade,
...
ScopedAStatus BeanpodKeyMintDevice::deleteKey(const vector<uint8_t>& keyBlob) {
...
ScopedAStatus BeanpodKeyMintDevice::deleteAllKeys() {
...
ScopedAStatus BeanpodKeyMintDevice::destroyAttestationIds() {
	return kmError2ScopedAStatus(KM_ERROR_UNIMPLEMENTED);
}

ScopedAStatus BeanpodKeyMintDevice::begin(KeyPurpose purpose, const vector<uint8_t>& keyBlob,
	...
	result->operation =
		ndk::SharedRefBase::make<BeanpodKeyMintOperation>(impl_, response.op_handle);
	return ScopedAStatus::ok();
}

ScopedAStatus BeanpodKeyMintDevice::deviceLocked(
	...
	DeviceLockedResponse response(impl_->message_version());
	...
	impl_->DeviceLocked(request, &response);
	return kmError2ScopedAStatus(response.error);
}

ScopedAStatus BeanpodKeyMintDevice::earlyBootEnded() {
	EarlyBootEndedResponse response(impl_->message_version());
	impl_->EarlyBootEnded(&response);
	return kmError2ScopedAStatus(response.error);
}

ScopedAStatus BeanpodKeyMintDevice::convertStorageKeyToEphemeral(
					const std::vector<uint8_t>& /* storageKeyBlob */,
					std::vector<uint8_t>* /* ephemeralKeyBlob */) {
	return kmError2ScopedAStatus(KM_ERROR_UNIMPLEMENTED);
}

ScopedAStatus BeanpodKeyMintDevice::getKeyCharacteristics(
	...
	*keyCharacteristics = convertKeyCharacteristics(securityLevel_,
					emptySet,
					response.unenforced,
					response.enforced,
					/* include_keystore_enforced = */ false);
...
IKeyMintDevice* CreateKeyMintDevice(BeanpodKeymaster* impl) {
	return ::new BeanpodKeyMintDevice(impl);
}
