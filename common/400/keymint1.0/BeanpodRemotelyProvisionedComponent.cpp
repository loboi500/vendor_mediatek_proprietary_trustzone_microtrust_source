...
constexpr auto STATUS_FAILED = BeanpodRemotelyProvisionedComponent::STATUS_FAILED;
...
BeanpodRemotelyProvisionedComponent::BeanpodRemotelyProvisionedComponent(
		std::shared_ptr<keymint::BeanpodKeyMintDevice>& keymint) {
	impl_ = keymint->getKeymasterImpl();
}

ScopedAStatus BeanpodRemotelyProvisionedComponent::getHardwareInfo(RpcHardwareInfo* info) {
	info->versionNumber = 1;
	info->rpcAuthorName = "Beanpod";
	info->supportedEekCurve = RpcHardwareInfo::CURVE_25519;
	return ScopedAStatus::ok();
}

ScopedAStatus BeanpodRemotelyProvisionedComponent::generateEcdsaP256KeyPair(bool testMode,
...
ScopedAStatus BeanpodRemotelyProvisionedComponent::generateCertificateRequest(
