...
int bp_keymaster_call(uint32_t cmd, void* in, uint32_t in_size, uint8_t* out, uint32_t* out_size) {
	if (handle_ < 0) {
		ALOGE("not connected\n");
		return -EINVAL;
	}
...
keymaster_error_t bp_keymaster_send(uint32_t command,
			...
			keymaster::KeymasterResponse* rsp)
{
	...
	if (rc < 0) {
		...
		ALOGI("cmd:%x Received %d byte response\n", command, rsp_size);
	}
...
keymaster_error_t bp_keymaster_send_no_request(uint32_t command,
						keymaster::KeymasterResponse* rsp)
{
	uint8_t recv_buf[BP_KEYMASTER_RECV_BUF_SIZE];
	keymaster::Eraser recv_buf_eraser(recv_buf, BP_KEYMASTER_RECV_BUF_SIZE);
	...
	if (rc < 0) {
		...
		ALOGI("BeanpodKeymaster cmd:%x Received %d byte response\n", command, rsp_size);
	}
