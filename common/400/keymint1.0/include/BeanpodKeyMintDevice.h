...
class BeanpodKeyMintDevice : public BnKeyMintDevice {
public:
	explicit BeanpodKeyMintDevice(BeanpodKeymaster* impl);
	virtual ~BeanpodKeyMintDevice();
	...
	ScopedAStatus
	getKeyCharacteristics(const std::vector<uint8_t>& keyBlob,
				const std::vector<uint8_t>& appId,
				const std::vector<uint8_t>& appData,
				std::vector<KeyCharacteristics>* keyCharacteristics) override;

	shared_ptr<BeanpodKeymaster>& getKeymasterImpl() { return impl_; }
